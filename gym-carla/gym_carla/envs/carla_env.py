...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    self.observation_space = spaces.Box(low=0, high=255, shape=(5, self.obs_size, self.obs_size), dtype=np.uint8)
  ...
  def _get_obs(self):
    ...
    self.lidar_img = np.mean(self.lidar_img, axis=2)
    self.lidar_img = self.lidar_img.astype(np.uint8)

    lidar_arr = np.zeros((1, self.obs_size, self.obs_size), dtype=np.uint8)

    lidar_arr[0] = resize(self.lidar_img, (self.obs_size, self.obs_size), preserve_range=True)
    lidar_arr = lidar_arr.astype(np.uint8)
    lidar_surface = grayscale_to_display_surface(lidar_arr[0], self.display_size)
    ...
    #camera = np.where(camera < 216, 0, 255)
    ...
    camera_surface2 = grayscale_to_display_surface(camera[1], self.display_size)
    self.display.blit(camera_surface2, (self.display_size * 2, 0))

    camera_surface3 = grayscale_to_display_surface(camera[2], self.display_size)
    self.display.blit(camera_surface3, (self.display_size * 4, 0))

    camera_surface4 = grayscale_to_display_surface(camera[3], self.display_size)
